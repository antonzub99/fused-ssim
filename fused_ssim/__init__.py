...
class FusedSSIMMapBfloat16(torch.autograd.Function):
    @staticmethod
    def forward(ctx, C1, C2, img1, img2, padding="same", train=True):
        ssim_map, dm_dmu1, dm_dsigma1_sq, dm_dsigma12 = fusedssim_bfloat16(C1, C2, img1, img2, train)
    ...
    def backward(ctx, opt_grad):
        ...
        if padding == "valid":
            dL_dmap = torch.zeros_like(img1)
            dL_dmap[:, :, 5:-5, 5:-5] = opt_grad
        grad = fusedssim_bfloat16_backward(C1, C2, img1, img2, dL_dmap, dm_dmu1, dm_dsigma1_sq, dm_dsigma12)
        return None, None, grad, None, None, None
    
class FusedSSIMMapHalf(torch.autograd.Function):
    @staticmethod
    def forward(ctx, C1, C2, img1, img2, padding="same", train=True):
        ssim_map, dm_dmu1, dm_dsigma1_sq, dm_dsigma12 = fusedssim_half(C1, C2, img1, img2, train)
    ...
    def backward(ctx, opt_grad):
        ...
        if padding == "valid":
            dL_dmap = torch.zeros_like(img1)
            dL_dmap[:, :, 5:-5, 5:-5] = opt_grad
        grad = fusedssim_half_backward(C1, C2, img1, img2, dL_dmap, dm_dmu1, dm_dsigma1_sq, dm_dsigma12)
        return None, None, grad, None, None, None
...
def fused_ssim(img1, img2, padding="same", train=True):
    ...
    elif img1.dtype == torch.bfloat16:
        map = FusedSSIMMapBfloat16.apply(C1, C2, img1, img2, padding, train)
    elif img1.dtype == torch.float16:
        map = FusedSSIMMapHalf.apply(C1, C2, img1, img2, padding, train)
    else:
        raise ValueError(f"Unsupported dtype {img1.dtype}")
    return map.mean()
